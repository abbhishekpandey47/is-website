...
const page = () => {
    ...
    const paginationAlgo = useCallback(
        (filteredData) => {
            ...
            if (myrtr[myrtr.length - 1] >= Math.ceil(filteredData.length / Number(totalItem))) {
                myrtr.pop()
            }
            if (myrtr[myrtr.length - 2] >= Math.ceil(filteredData.length / Number(totalItem))) {
                myrtr.pop()
            }

            setCardInfo(filteredData.slice((pageNum - 1) * totalItem, (pageNum - 1) * totalItem + Number(totalItem)))
            setPaginArr(myrtr)
        },
        [pageNum, totalItem],
    );
                ...
                <Pagination>
                    <PaginationContent>
                        {Number(pageNum) - 1 > 0 && (<PaginationItem>
                            <PaginationPrevious href={`?page=${Number(pageNum) - 1}&totalItem=${totalItem}`} />
                        </PaginationItem>)}
                        {paginArr.map((item, index) => {
                            return (
                                <PaginationItem key={item}>
                                    <PaginationLink isActive={pageNum == item ? true : false} href={`?page=${item}&totalItem=${totalItem}`}>{item}</PaginationLink>
                                </PaginationItem>
                            )
                        })}
                        ...
                        {Number(pageNum) + 1 <= paginArr[paginArr.length - 1] && (
                            <>
                                ...
                                <PaginationItem>
                                    <PaginationNext href={`?page=${Number(pageNum) + 1}&totalItem=${totalItem}`} />
                                </PaginationItem></>)}
                    </PaginationContent>
                </Pagination>
            </div>
