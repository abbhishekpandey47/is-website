...
export default function PostAnalyticsTable({ threads = [], initialCount = 10 }) {
  ...
  return (
    <Card className="animate-slide-up shadow-md">
      <CardHeader>
        <CardTitle className="text-xl font-semibold text-foreground">Posts Analytics</CardTitle>
      </CardHeader>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-3 px-2 text-sm font-medium text-foreground-muted">Posts</th>
            ...
            <tbody>
              {visibleThreads.length === 0 ? (
                <tr><td colSpan={5} className="py-8 text-center text-foreground-muted">No Posts available.</td></tr>
              ) : (
                visibleThreads.map((thread, index) => (
                  <tr key={thread.id ?? index} className="border-b border-border-muted hover:bg-interactive-hover transition-colors">
