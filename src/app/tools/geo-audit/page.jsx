...
const DUMMY_RESULTS = {
  summary: { totalPages:12, avgScore:58, pagesBelow30:2, pagesBelow50:5, weakestSignal:"citations" },
  topPages: [
  ...
  rewrites: [
    { url:"https://infrasity.com/blog/ai-agent-content-strategy", rewrites:[
      ...
      {signal:"directness",priority:"high",original:"In this article we explore how B2B SaaS companies can think about their content strategy.",rewrite:"Convert all H2 headings to answer-first question format with immediate direct answers",rationale:"Rewrite H2s as questions: What Are AI Agents becomes How Do AI Agents Change Discovery"},
      ...
      {signal:"citations",priority:"medium",original:"Not present",rewrite:"Publish original data on citation rates bot traffic or conversion by AI source",rationale:"Original proprietary data is cited 4x more than rephrased third-party claims"},
    ]},
    { url:"https://infrasity.com/blog/geo-vs-seo", rewrites:[
      {signal:"citations",priority:"high",original:"Traditional SEO focuses on ranking in Google, while GEO focuses on AI engines.",rewrite:"Add Forrester or Gartner citation: 62% of enterprise buyers used AI for first research touchpoint",rationale:"Cited statistics are quoted by AI engines 5x more than uncited claims"},
      ...
      {signal:"directness",priority:"medium",original:"There are several differences between GEO and SEO that marketers should understand.",rewrite:"Rewrite opening as direct answer: The core difference — SEO wins rankings, GEO wins AI citations",rationale:"Answer-first writing in the first sentence makes the page quotable for comparison queries"},
    ]},
    { url:"https://infrasity.com/services/geo-optimization", rewrites:[
      ...
      {signal:"citations",priority:"high",original:"GEO is becoming the most important channel for B2B SaaS marketing.",rewrite:"Add Forrester citation: 62% of enterprise software buyers used AI as first research touchpoint in 2025",rationale:"Service pages with cited statistics are 4x more likely to be quoted in vendor comparisons"},
    ]},
    { url:"https://infrasity.com/blog/perplexity-citations", rewrites:[
      {signal:"citations",priority:"high",original:"Getting cited by Perplexity requires a different approach than traditional SEO.",rewrite:"Add link to Perplexity publisher guidelines and Search Engine Land analysis showing 5x citation lift",rationale:"Links to official platform docs create the authoritative citation chain AI engines trust"},
      {signal:"entities",priority:"high",original:"Not present",rewrite:"Add definition: Perplexity AI is an answer engine processing 100M+ monthly queries that cites sources inline",rationale:"Defining the subject entity in the first 100 words is the highest-impact GEO move for topical pages"},
    ]},
    { url:"https://infrasity.com/case-studies/saas-geo-results", rewrites:[
      ...
      {signal:"directness",priority:"medium",original:"In this case study we walk through the process we used to improve AI visibility.",rewrite:"Open with direct result: We increased AI citations 3x in 90 days by fixing citations, H2 structure and FAQ schema",rationale:"Answer-first case study intro is quoted in AI responses to how to increase AI citations queries"},
    ]},
  ],
};
...
function ScoreHeroCard({summary}){
  ...
  return(
    <div style={{border:"1px solid #333",borderRadius:10,padding:"24px 28px",background:"#1a1a1a",marginBottom:24}}>
      <div style={{display:"flex",alignItems:"flex-start",gap:28,flexWrap:"wrap"}}>
        <div style={{flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"baseline",gap:4}}>
            ...
            <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:18,color:"#a8a29e"}}>/100</span>
          </div>
        </div>
        <div style={{flex:1,minWidth:180,paddingTop:8}}>
          <p style={{fontSize:14,color:"#c8c4bc",lineHeight:1.65,fontFamily:"'DM Sans',sans-serif",marginBottom:16}}>
      ...
      {order.map(sig=>{
        ...
        return(
          <div key={sig} style={{display:"flex",alignItems:"center",gap:14,marginBottom:10}}>
            <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:"#c8c4bc",width:110,flexShrink:0}}>{SIGNAL_DISPLAY[sig]||sig}</span>
  ...
  return(
    <div style={{marginBottom:22}}>
      ...
      {rewrites.slice(0,4).map((rw,i)=>{
        ...
        return(
          <div key={i} style={{display:"flex",gap:10,marginBottom:10,alignItems:"flex-start",background:"#222",borderRadius:7,padding:"12px 14px",border:"1px solid #333"}}>
            ...
            <div>
              ...
              {rw.rationale&&<div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#a8a29e",lineHeight:1.5}}>{rw.rationale.slice(0,95)}{rw.rationale.length>95?"…":""}</div>}
            </div>
          </div>
        );
      })}
    ...
    <div style={{marginBottom:24}}>
      ...
      {actions.map((a,i)=>(
        <div key={i} style={{display:"flex",gap:14,marginBottom:8,alignItems:"flex-start"}}>
          <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#a8a29e",flexShrink:0,width:20,paddingTop:2}}>{String(i+1).padStart(2,"0")}</span>
          <span style={{fontSize:14,color:"#f5f0e8",lineHeight:1.65,fontFamily:"'DM Sans',sans-serif"}}>{a}</span>
        </div>
      ))}
    </div>
  );
...
function CTAStrip(){
  return(
    <div style={{borderTop:"1px solid #333",paddingTop:18,marginTop:4,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
      <div>
        <div style={{fontSize:15,color:"#ede9e1",fontFamily:"'DM Sans',sans-serif",fontWeight:500,marginBottom:4}}>Want us to fix this?</div>
        <div style={{fontSize:12,color:"#a8a29e",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.5}}>Infrasity implements GEO for B2B SaaS and drives traffic or conversion by AI source</div>
      </div>
...
function PageCard({page,rewriteResult,rank}){
  ...
  return(
    <div style={{border:"1px solid #333",borderRadius:8,marginBottom:10,overflow:"hidden",background:open?"#111":"#1a1a1a"}}>
      <div onClick={()=>setOpen(!open)} style={{padding:"14px 18px",cursor:"pointer",borderBottom:open?"1px solid #333":"none"}}>
        <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#c8c4bc",marginBottom:4}}>{category}</div>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:14}}>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,fontWeight:500,color:"#ede9e1",fontFamily:"'DM Sans',sans-serif",marginBottom:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{page.title||shortUrl(page.url)}</div>
            <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#6b6760",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{page.url}</div>
          </div>
          <div style={{width:34,height:34,borderRadius:"50%",border:`1.5px solid ${color}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,background:`${color}18`,marginTop:2}}>
...
/* Single current URL — no list */
function CurrentURL({urls}){
  if(!urls||!urls.length)return null;
  return(
    <div style={{marginTop:12}}>
      <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#d4980a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
        → {shortUrl(urls[urls.length-1])}
      </div>
    </div>
  );
}
  ...
  return(
    <div>
      <div style={{marginBottom:20}}>
        <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:"0.08em",color:"#c8c4bc",marginBottom:6}}>GEO Audit Report</div>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
          <div>
            ...
            <p style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#c8c4bc",margin:0}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={onDownload} style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#c8c4bc",border:"1px solid #444",borderRadius:6,padding:"7px 14px",background:"none",cursor:"pointer"}}>↓ report.md</button>
            <button onClick={onReset} style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#c8c4bc",border:"1px solid #444",borderRadius:6,padding:"7px 14px",background:"none",cursor:"pointer"}}>+ New audit</button>
          </div>
        </div>
      </div>
      ...
      {results.topPages?.length>0&&(
        <>
          <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:"#c8c4bc",marginBottom:10}}>Page breakdown</div>
  ...
  async function runAudit(){
    ...
    try{
      const res=await fetch("/api/geo-audit",{
        ...
        body:JSON.stringify({domain:domain.trim(),pages:pages.filter(p=>p.url||p.content),maxPages:50,topN:10}),
      });
      ...
      while(true){
        ...
        for(const chunk of chunks){
          ...
          if(data.event==="progress"){
            ...
            if(data.urls&&data.urls.length) setStepUrls(data.urls);
          }
          if(data.event==="error")throw new Error(data.message);
          if(data.event==="complete"){
            setMarkdown(data.markdown||"");setResults(data);
  ...
  return(
    <div style={{maxWidth:780,margin:"0 auto",padding:"100px 32px 100px",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <>
        ...
        <p style={{fontSize:15,color:"#c8c4bc",maxWidth:520,lineHeight:1.65,marginBottom:36}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:40}}>
          {SIGNAL_PILLS.map(s=>(
            <div key={s.label} style={{border:`1px solid ${s.border}`,borderRadius:6,padding:"10px 14px",fontFamily:"'IBM Plex Mono',monospace",fontSize:11.5,color:"#ede9e1",display:"flex",alignItems:"center",gap:10,background:s.bg}}>
              <span style={{color:s.dot,fontSize:14,flexShrink:0}}>•</span>{s.label}
            </div>
          ))}
        </div>

        <div style={{border:"1px solid #3a3a3a",borderRadius:10,padding:28,background:"#1a1a1a",marginBottom:14}}>
          <div style={{marginBottom:24}}>
            <label htmlFor="geo-domain" style={{display:"block",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:"#c8c4bc",marginBottom:8}}>Domain / Company name</label>
            <input id="geo-domain" type="text" placeholder="e.g. futuresearch.ai or InventDB"
              value={domain} onChange={e=>setDomain(e.target.value)}
              style={{width:"100%",background:"#222",border:"1px solid #3a3a3a",borderRadius:6,padding:"12px 16px",fontFamily:"'IBM Plex Mono',monospace",fontSize:13,color:"#ede9e1",outline:"none",boxSizing:"border-box"}}/>
          </div>
          <div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <label style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:"#c8c4bc"}}>Pages to audit</label>
              <button onClick={addPage} disabled={pages.length>=5} style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#c8c4bc",border:"1px solid #444",borderRadius:5,padding:"5px 12px",background:"none",cursor:"pointer"}}>+ Add another page</button>
            </div>
            {pages.map((page,i)=>(
              <div key={i} style={{border:"1px solid #333",borderRadius:8,padding:14,marginBottom:8,background:"#222"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#a8a29e"}}>Page {i+1}</span>
                  {pages.length>1&&<button onClick={()=>removePage(i)} style={{background:"none",border:"none",cursor:"pointer",color:"#a8a29e",fontSize:18,lineHeight:1}}>×</button>}
                </div>
                <input type="text" placeholder="Page URL or path (optional)"
                  value={page.url} onChange={e=>updatePage(i,"url",e.target.value)}
                  style={{width:"100%",background:"#1e1e1e",border:"1px solid #333",borderRadius:5,padding:"10px 13px",fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:"#ede9e1",marginBottom:8,boxSizing:"border-box",outline:"none"}}/>
                <textarea placeholder="Paste full page content — title, headings, body text, any FAQs..."
                  value={page.content} onChange={e=>updatePage(i,"content",e.target.value)} rows={5}
                  style={{width:"100%",background:"#1e1e1e",border:"1px solid #333",borderRadius:5,padding:"10px 13px",fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:"#ede9e1",resize:"vertical",boxSizing:"border-box",outline:"none"}}/>
              </div>
            ))}
          </div>
        </div>

        <p style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#a8a29e",marginBottom:24}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
          ...
          {/* <button onClick={loadPreview} style={{display:"inline-flex",alignItems:"center",gap:8,background:"none",color:"#c8c4bc",fontFamily:"'IBM Plex Mono',monospace",fontSize:12,padding:"13px 20px",borderRadius:7,border:"1px solid #444",cursor:"pointer"}}>
            Preview demo
          </button> */}
          <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#a8a29e"}}>Powered by OpenAI · ~20 sec</span>
        </div>
      ...
      {/* ── LOADING ── */}
      {loading&&(
        <div style={{marginTop:36,border:"1px solid #3a3a3a",borderRadius:8,padding:28,background:"#1a1a1a"}}>
            ...
            {STEPS.map((step,i)=>{
              ...
              return(
                <li key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"5px 0"}}>
                  <span style={{width:6,height:6,borderRadius:"50%",flexShrink:0,display:"inline-block",background:st==="done"?"#a87a08":st==="active"?"#d4980a":"#444"}}/>
                  <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:12,fontWeight:st==="active"?500:400,color:st==="done"?"#a8a29e":st==="active"?"#ede9e1":"#6b6760"}}>{step}</span>
                </li>
              );
            })}
          </ul>
          ...
          {stepIndex>=0&&(
            <>
              <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#c8c4bc",letterSpacing:"0.04em"}}>{stepMsg||STEP_DETAILS[stepIndex]?.sub}</div>
              {/* Single current URL — updates live */}
              {stepUrls.length>0&&(stepIndex===0||stepIndex===1||stepIndex===4)&&(
                <CurrentURL urls={stepUrls}/>
              )}
            </>
          )}
        </div>
      ...
      {results&&!loading&&(
        <div ref={resultsRef}>
          <hr style={{border:"none",borderTop:"1px solid #3a3a3a",margin:"36px 0"}}/>
          <ResultsView results={results} domain={domain} onReset={resetAudit} onDownload={downloadReport}/>
        </div>
      )}

      <style>{`
        input::placeholder, textarea::placeholder { color: #6b6760; }
