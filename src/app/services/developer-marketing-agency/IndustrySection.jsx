...
// Generate unique ID for each card instance
let cardIdCounter = 0;

const IndustryCard = memo(({ icon, title, description, href }) => {
  const cardRef = useRef(null);
  const workerRef = useRef(null);
  const cardId = useRef(`card-${cardIdCounter++}`);
  ...
  useEffect(() => {
    ...
    try {
      workerRef.current = new Worker('/workers/mouseCalculations.worker.js');
      
      workerRef.current.onmessage = (e) => {
        const { type, result } = e.data;
        if (type === 'POSITION_CALCULATED' && cardRef.current) {
          // Update CSS variables for smooth animation
          cardRef.current.style.setProperty("--mouse-x", `${result.x}px`);
          cardRef.current.style.setProperty("--mouse-y", `${result.y}px`);
    ...
    // Throttled mouse move handler using RAF for 60fps performance
...
const IndustrySection = memo(() => {
 ...
 // Memoize card data to prevent recreating on every render
 ...
 const visibleCards = useMemo(() => {
   return showAll ? cardData : cardData.filter(card => card.alwaysShow);
 }, [showAll, cardData]);
