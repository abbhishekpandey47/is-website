"use client";
import { AlertCircle, Clock, Bug, Search, CheckCircle2, Star, Lightbulb, X } from "lucide-react";
...
const afterItems = [
  { icon: <CheckCircle2 className="w-6 h-6 text-green-400" />, text: "CI always passes after merge" },
  { icon: <Lightbulb className="w-6 h-6 text-green-400" />, text: "Drift from main automatically detected" },
  { icon: <Star className="w-6 h-6 text-green-400" />, text: "Retries are automatic, tracked, and reduced" },
  { icon: <Lightbulb className="w-6 h-6 text-green-400" />, text: "Flaky jobs detected, flagged, and monitored" },
  { icon: <Lightbulb className="w-6 h-6 text-green-400" />, text: "Job-level visibility helps pinpoint what broke" },
];
            ...
            <div>
              ...
              <ul className="space-y-5">
                {beforeItems.map((item, i) => (
                  ...
                  >
                   <div class="flex items-center justify-center w-8 h-8 rounded-md bg-zinc-800/60 shadow-inner ring-1 ring-white/10">
      <svg class="text-white w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
  {item.icon}
      </svg>
    </div>
                ...
                {afterItems.map((item, i) => (
                  <li
                    ...
                    className="flex items-center gap-3 w-full rounded-xl
                      ...
                      p-3 transition-transform"
                  >
                    <div className="flex items-center justify-center w-8 h-8 rounded-md bg-zinc-800/60 shadow-inner ring-1 ring-white/10">
                      {item.icon}
                    </div>
                    <span className="text-sm font-medium">{item.text}</span>
                  </li>
