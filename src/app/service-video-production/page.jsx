...
const YtCard = ({ handleOnOpen, item, setYtContentLink }) => {
  ...
  return (
    <div onClick={handleOnYtCard} className="w-96 flex items-center rounded-md relative cursor-pointer hover:scale-105 transition-all bg-black ytCardDiv">
      ...
      <div onClick={handleOnOpen} className="absolute w-full h-full bg-black top-0 rounded-md opacity-[0] hover:opacity-[0.8] transition-all">
        <div className="w-full h-full flex justify-center items-center absolute"><PlayCircleOutlined className="text-3xl" /></div>
        <div className="p-5 quicksand-bold h-full flex flex-col justify-end"><p >Watch Now <ArrowRightOutlined /></p></div>
      </div>
...
const PopupVideo = ({ handleOnClose, ytContentLink }) => {
  return (
    <>
      <div onClick={handleOnClose} className="fixed hidden bg-black/10 left-0 top-0 right-0 bottom-0 z-30 backdrop-blur-lg" id="popUpDisplayId"></div>
      <div className="fixed left-0 top-0 right-0 bottom-0 z-40 backdrop-blur-lg hidden" id="videoShowId">
        <div className='flex justify-center'>
          <div className='fixed top-[50%] left-[50%] origin-center transform -translate-x-1/2 -translate-y-1/2 bg-zinc-800 p-5 rounded-lg'>
            ...
            <div className="pb-5"><hr className="border-[#999]" /></div>
            <div>
              {ytContentLink.category === "Podcasts" ? (<iframe
...
const page = () => {
  ...
  const scrollToSection = () => {
    const youtubeSectionIdService = document.getElementById("youtubeSectionIdService");
  ...
  return (
    <div className='max-w-[1566px] mx-auto overflow-hidden'>
      <div className="pt-40 space-y-9 relative blogDiv">
        ...
        <div className="flex justify-center"><button className="btn bg-btnprimary hover:bg-btnprimaryhov text-white text-center relative z-[12px]">Get Started</button></div>
      </div>
      ...
      <div className="flex justify-center w-full flex-col items-center">
        ...
        <div className="max-w-[1450px] mx-auto flex justify-center flex-col items-center gap-5 text-center">
          ...
          <div className=' w-6/5 m-auto flex flex-col gap-4 max-sm:gap-2'>
            <div className='w-full text-center pt-20 text-3xl quicksand-bold text-[wheat]'><h2>Steps for Video Production</h2></div>
          </div>
          <div className='pt-5' />
          <MarqueeDemo />
        </div>
      ...
      <div className="max-w-[1450px] mx-auto justify-center flex pt-12">
        <div className="w-[90%] flex flex-wrap gap-5 justify-center" id='youtubeSectionIdService'>
          {ytInfo.map((item) => {
            return (<YtCard ytContentLink={ytContentLink} setYtContentLink={setYtContentLink} handleOnOpen={handleOnOpen} item={item} />)
          })}
        </div>
      </div>
